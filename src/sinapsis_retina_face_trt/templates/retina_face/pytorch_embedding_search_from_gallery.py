...
def make_metadata(
    ...
    bounding_boxes: list[BoundingBox | None] | None = None,
) -> pl.DataFrame:
...
class PytorchEmbeddingSearch(Template):
    ...
    def force_build_from_dir(self) -> bool:
        ...
        """
        full_path = os.path.join(self.root_dir, self.attributes.gallery_file)
        return self.attributes.force_build_from_dir or not os.path.exists(full_path)
    ...
    def load_from_file(self) -> EmbeddingGallery:
        ...
        gallery_full_path = os.path.join(self.root_dir, self.attributes.gallery_file)
    ...
    def infer_model(self, img_packet: ImagePacket) -> np.ndarray:
        ...
        embedding: np.ndarray = out_data_container.images[0].embedding  # ty: ignore[invalid-assignment]
        return embedding
    ...
    def build_from_dir(self) -> EmbeddingGallery:
        ...
        for img_packet in FolderImageDatasetCV2(
            ...
            img_packet = cast(ImagePacket, img_packet)
            if self.attributes.use_face_detector_for_gallery_creation:
    ...
    def _get_top_k_matches(self, query: np.ndarray, gallery: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.attributes.metric == "euclidean":
            ...
            dist = torch.cosine_similarity(gallery, torch.Tensor(query), dim=1)
